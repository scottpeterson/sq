...
import com.football.assets.dataLayer.R
...
sealed class DataSourceError : SquareError() {
    ...
    data class JsonDataException(val exception: com.squareup.moshi.JsonDataException) :
        DataSourceError()
    ...
    override fun toAppetizerMessage(context: Context): String {
        return when (this) {
            ...
            is JsonDataException -> context.getString(
                R.string.error_api_json_data,
                exception.message
            )

            is Unknown -> context.getString(R.string.error_api_unknown)
            is RequestFailed -> this.errorMessage
                ?: context.getString(R.string.error_api_request_failed, this.code)
        }
    }
}
